...
package object reflect {
  ...
  implicit def method2EnhancedMethod(m: Method): EnhancedMethod = class2EnhancedClass(m.getDeclaringClass)(m)

  implicit def f0ToEM(f: Function0[_]): Option[EnhancedMethod] = {
    class2EnhancedClass(f.getClass).methods.find(m => m.name == "apply" && m.args.length == 0)
  }

  implicit def f1ToEM(f: Function1[_, _]): Option[EnhancedMethod] = {
    class2EnhancedClass(f.getClass).methods.find(m => m.name == "apply" && m.args.length == 1)
  }

  implicit def f2ToEM(f: Function2[_, _, _]): Option[EnhancedMethod] = {
    class2EnhancedClass(f.getClass).methods.find(m => m.name == "apply" && m.args.length == 2)
  }

  implicit def f3ToEM(f: Function3[_, _, _, _]): Option[EnhancedMethod] = {
    class2EnhancedClass(f.getClass).methods.find(m => m.name == "apply" && m.args.length == 3)
  }

  implicit def f4ToEM(f: Function4[_, _, _, _, _]): Option[EnhancedMethod] = {
    class2EnhancedClass(f.getClass).methods.find(m => m.name == "apply" && m.args.length == 4)
  }

  implicit def f5ToEM(f: Function5[_, _, _, _, _, _]): Option[EnhancedMethod] = {
    class2EnhancedClass(f.getClass).methods.find(m => m.name == "apply" && m.args.length == 5)
  }

  implicit def f6ToEM(f: Function6[_, _, _, _, _, _, _]): Option[EnhancedMethod] = {
