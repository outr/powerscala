...
object MapSupport extends Listenable {
  ...
  }.objectAlias(classOf[Map.Map1[_, _]], classOf[Map.Map2[_, _]], classOf[Map.Map3[_, _]], classOf[Map.Map4[_, _]], classOf[HashTrieMap[_, _]], classOf[ListMap[_, _]], getClass.getClassLoader.loadClass("scala.collection.immutable.ListMap$Node"), getClass.getClassLoader.loadClass("scala.collection.immutable.Map$EmptyMap$"))
...
class MapModifiableProcessor(name: String)(implicit listenable: Listenable) extends ModifiableProcessor[Map[String, Any]](name)(listenable, implicitly[Manifest[Map[String, Any]]]) {
  def removeWhen(key: String, value: JValue) = on {
    case m if m.get(key).contains(value) => m - key
    case m => m
  }
